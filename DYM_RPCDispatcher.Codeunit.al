codeunit 84026 DYM_RPCDispatcher
{
    trigger OnRun()
    var
        ObjectType: Text;
    ...
    begin
        ...
        case ObjectType of ConstMgt.ROT_Codeunit: CODEUNIT.Run(LowLevelDP.Text2Integer(ObjectNo));
        ConstMgt.ROT_Report: REPORT.Run(LowLevelDP.Text2Integer(ObjectNo));
        ConstMgt.ROT_Command: Dispatch;
        else
            Error(Text001, ObjectType);
        end;
    end;
    var XMLMgt: Codeunit DYM_XMLManagement;
    ...
    begin
        ...
        case Command of 'CreateSalesHeader': CreateSalesHeader;
        'CreateSalesLine': CreateSalesLine;
        'UpdateSalesLine': UpdateSalesLine;
        'DeleteSalesLine': DeleteSalesLine;
        'PrintSalesHeader': PrintSalesHeader;
        'OurFancyFunction': OurFancyFunction;
        'Test': Test();
        else
    ...
    begin
        ...
        if(CacheMgt.GetRPCCache(enum::DYM_PacketDirection::Push, 'LocationCode', ConstMgt.RPC_SystemRecID()) <> '')then SalesLine.Validate("Location Code", CacheMgt.GetRPCCache(enum::DYM_PacketDirection::Push, 'LocationCode', ConstMgt.RPC_SystemRecID()))
        else
