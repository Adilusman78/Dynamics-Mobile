codeunit 84029 DYM_ActiveRecManagement
{
    trigger OnRun()
    begin
    end;
    ...
    #region ActiveTable
    procedure FindTable()Result: Boolean begin
        Clear(Result);
    ...
    procedure NextTable()Result: Boolean begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        exit(ActiveTableRDL.Next = 1);
    end;
    procedure GetTableName(): Text begin
        exit(ActiveTableRDL."Table Name");
    end;
    procedure TableRecordsCount()Result: Integer var
    ...
    #endregion 
    #region ActiveRec
    procedure ReadTable(TableName: Text)Result: Boolean var
    ...
    procedure FindRecord()Result: Boolean begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
    ...
    procedure FindTable(TableName: Text): Boolean begin
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
    ...
    procedure NextRecord()Result: Boolean begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        TestTable;
        exit(ActiveRecRDL.Next = 1);
    end;
    procedure GetRecordNo(): Integer begin
    ...
    procedure GetActiveRecContext()Result: Text begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        TestTable;
    ...
    #endregion 
    #region ActiveField
    procedure FindField(TableName: Text): Boolean begin
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        ActiveFieldRDL.Reset;
        ActiveFieldRDL.SetRange("Sync Log Entry No.", SLE);
    ...
    procedure NextField(): Boolean begin
        exit(ActiveFieldRDL.Next = 1);
    end;
    procedure GetFieldName(): Text begin
        exit(ActiveFieldRDL."Field Name");
    end;
    #endregion 
    #region Field Access
    procedure TextField(FieldName: Text)Result: Text var
    ...
    procedure BooleanField(FieldName: Text): Boolean begin
        exit(LowLevelDP.Text2Boolean(TextField(FieldName)));
    end;
    procedure IntegerField(FieldName: Text): Integer begin
        exit(LowLevelDP.Text2Integer(TextField(FieldName)));
    end;
    procedure BigIntegerField(FieldName: Text): BigInteger begin
        exit(LowLevelDP.Text2BigInteger(TextField(FieldName)));
    end;
    procedure DecimalField(FieldName: Text): Decimal begin
        exit(LowLevelDP.Text2Decimal(TextField(FieldName)));
    end;
    procedure DateField(FieldName: Text): Date begin
        exit(DT2Date(LowLevelDP.Text2DateTime(TextField(FieldName))));
    end;
    procedure ODDateField(FieldName: Text): Date begin
        exit(LowLevelDP.ODText2Date(TextField(FieldName)));
    end;
    procedure TimeField(FieldName: Text): Time begin
        exit(DT2Time(LowLevelDP.Text2DateTime(TextField(FieldName))));
    end;
    procedure DateTimeField(FieldName: Text): DateTime begin
    ...
    procedure OptionField(FieldName: Text; FieldPos: Text): Integer begin
        EXIT(LowLevelDP.Text2OptionValue(FieldPos, TextField(FieldName)));
    end;
    #endregion 
    #region Attributes
    procedure GetGlobalAttribute(AttributeName: Text)Result: Text begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        exit(GetRawDataLogEntry(enum::DYM_RawDataLogEntryType::"Global Attribute", '', 0, '', AttributeName));
    end;
    procedure CheckGlobalAttribute(AttributeName: Text; AttributeValue: Text): Boolean begin
        exit(GetGlobalAttribute(AttributeName) = AttributeValue);
    end;
    procedure GetTableAttribute(AttributeName: Text)Result: Text var
    ...
    procedure CheckTableAttribute(AttributeName: Text; AttributeValue: Text): Boolean var
        TableAttributeRDL: Record DYM_RawDataLog;
    begin
        exit(GetTableAttribute(AttributeName) = AttributeValue);
    end;
    procedure GetRecordAttribute(AttributeName: Text)Result: Text begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        TestTable;
    ...
    procedure CheckRecordAttribute(AttributeName: Text; AttributeValue: Text): Boolean begin
        exit(GetRecordAttribute(AttributeName) = AttributeValue);
    end;
    procedure GetFieldAttribute(FieldName: Text; AttributeName: Text)Result: Text begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        TestTable;
    ...
    procedure CheckFieldAttribute(FieldName: Text; AttributeName: Text; AttributeValue: Text): Boolean begin
        exit(GetFieldAttribute(FieldName, AttributeName) = AttributeValue);
    end;
    #endregion 
    #region Fields
    procedure CheckField(FieldName: Text)Result: Boolean begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        if(not CheckTable())then exit(false);
    ...
    procedure SetFieldFilter(FieldName: Text; FilterValue: Text)Result: BigInteger var
        NextEntry: Integer;
    begin
        Clear(Result);
    ...
    procedure SetRecordAttributeFilter(AttributeName: Text; FilterValue: Text)Result: BigInteger begin
        Clear(Result);
        TestTable;
        exit(InsertFilterRawDataLogEntry(enum::DYM_RawDataLogEntryType::"Record Attribute", 0, '', AttributeName, FilterValue, false));
    end;
    procedure SetRecordMissingAttributeFilter(AttributeName: Text)Result: BigInteger begin
        Clear(Result);
        TestTable;
        exit(InsertFilterRawDataLogEntry(enum::DYM_RawDataLogEntryType::"Record Attribute", 0, '', AttributeName, '', true));
    end;
    procedure SetFieldAttributeFilter(FieldName: Text; AttributeName: Text; FilterValue: Text)Result: BigInteger begin
        Clear(Result);
        TestTable;
    ...
    #endregion 
    #region Checks
    local procedure CheckTable(): Boolean begin
    ...
    #endregion 
    #region Raw Data Log
    local procedure GetRawDataLogEntry(EntryType: enum DYM_RawDataLogEntryType; TableName: Text; RecordNo: Integer; FieldName: Text; AttributeName: Text)Result: Text var
        RawDataLog: Record DYM_RawDataLog;
    begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
    ...
    local procedure CheckRawDataLogEntry(EntryType: enum DYM_RawDataLogEntryType; TableName: Text; RecordNo: Integer; FieldName: Text; AttributeName: Text)Result: Boolean var
        RawDataLog: Record DYM_RawDataLog;
    begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
    ...
    begin
        ...
        IF RawDataLog.FINDFIRST THEN BEGIN
            RawDataLog.CALCFIELDS("Blob Value");
            IF(RawDataLog."Blob Value".HASVALUE)THEN IF LowLevelDP.IsMedia(MediaFieldRef.RECORD.NUMBER, MediaFieldRef.NUMBER)THEN BEGIN
    ...
    local procedure InsertFilterRawDataLogEntry(EntryType: enum DYM_RawDataLogEntryType; RecordNo: Integer; FieldName: Text; AttributeName: Text; "Filter": Text; MissingAttribute: Boolean)Result: BigInteger var
        NextEntry: Integer;
    begin
        Clear(Result);
        TestTable;
    ...
    begin
        ...
        if RecordRDL.FindSet(false, false)then repeat Clear(ActiveRecRDL);
                ActiveRecRDL.Init;
    ...
    begin
        for FilterPass:=1 to 4 do begin
            case FilterPass of 1: EntryType:=enum::DYM_RawDataLogEntryType::Field.AsInteger();
            2: EntryType:=enum::DYM_RawDataLogEntryType::"Record Attribute".AsInteger();
            3: EntryType:=enum::DYM_RawDataLogEntryType::"Field Attribute".AsInteger();
            4: EntryType:=enum::DYM_RawDataLogEntryType::"Record Attribute".AsInteger();
            end;
            ...
            if FilterRDL.FindSet(false, false)then repeat FilteredRecRDL.SetRange("Field Name", FilterRDL."Field Name");
                    ...
                    if FilteredRecRDL.FindSet(false, false)then repeat ActiveRecRDL.Reset;
                            ActiveRecRDL.SetRange("Entry Type", enum::DYM_RawDataLogEntryType::Record);
                            ActiveRecRDL.SetRange("Table Name", ContextTableName);
                            ActiveRecRDL.SetRange("Record No.", FilteredRecRDL."Record No.");
                            case(FilterPass = 4)of true: ActiveRecRDL.SetRange("Filter Flag", true);
                            false: ActiveRecRDL.SetRange("Filter Flag", false);
                            end;
    ...
    #endregion 
    #region Position
    procedure ActiveRecPK2Text()Result: Text var
        FieldMap: Record DYM_MobileFieldMap;
    begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        if FindField(GetTableName)then repeat FieldMap.Reset;
