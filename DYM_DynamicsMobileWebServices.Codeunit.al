codeunit 84022 DYM_DynamicsMobileWebServices
{
    trigger OnRun()
    begin
    end;
    ...
    TableName: Text[250];
    RequestID: Text[250];
    procedure Read(DataIn: BigText; var DataOut: BigText)Result: Boolean var
        ReadPolicy: Text[250];
    begin
        Clear(Result);
    ...
    procedure Write(DataIn: BigText; var DataOut: BigText)Result: Boolean var
        EntryNo: Integer;
    ...
    begin
        ...
        if not _Node.AsXmlElement().IsEmpty()then if(_Node.AsXmlElement().InnerText <> '')then begin
                ...
                for i:=1 to _NodeList.Count()do begin
                    ...
                    if FieldMap.FindSet(false, false)then repeat FldRef:=RecRef.Field(FieldMap."Field No.");
