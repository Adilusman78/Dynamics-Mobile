codeunit 84001 DYM_GlobalDataProcess
{
    ...
    begin
        ...
        case Rec.Direction of Rec.Direction::Push: begin
            ...
            //END ;
            case Rec.Internal of true: GetSyncParamsInt(Rec, PullType, Challenge);
            false: GetSyncParams(PullType, Challenge);
            end;
    ...
    begin
        ...
        case Challenge of true: begin
            ...
            case PullType of PullType::Group: begin
    ...
    procedure InsertSyncLogEntry(FileName: Text[250]; Direction: enum DYM_PacketDirection; PacketType: enum DYM_PacketType; PullType: enum DYM_PullType; Internal: Boolean; WebServiceEntry: Boolean; packetContent: InStream)Result: Integer begin
        InsertSyncLogEntry(FileName, Direction, PacketType, PullType, Internal, WebServiceEntry, packetContent, CompanyName);
    end;
    procedure InsertSyncLogEntry(FileName: Text[250]; Direction: enum DYM_PacketDirection; PacketType: enum DYM_PacketType; PullType: enum DYM_PullType; Internal: Boolean; WebServiceEntry: Boolean; packetContent: InStream; _company: Text)Result: Integer var
    ...
    begin
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        Settings.Reset;
        case Direction of enum::DYM_PacketDirection::Push: Settings.SetFilter("Mandatory for", '%1|%2', Settings."Mandatory for"::Both, Settings."Mandatory for"::Push);
        enum::DYM_PacketDirection::Pull: Settings.SetFilter("Mandatory for", '%1|%2', Settings."Mandatory for"::Both, Settings."Mandatory for"::Pull);
        end;
        Settings.SetFilter("Applies-to Device Role", '%1|%2', DeviceRole.Code, '');
        if Settings.FindSet(false, false)then repeat SettingsMgt.TestSetting(Settings.Code);
    ...
    begin
        ...
        if not Challenge then begin
            ...
            */
            ...
            //END ;
            if ActiveTableRec.FindTable then repeat case ActiveTableRec.CheckGlobalAttribute(ConstMgt.WSC_ReadPolicy, ConstMgt.WSC_ReadPolicy_Standard)of true: ProcessPushTableStdRead(_Node, _ResponseRootNode);
                    false: ProcessPushTable(_Node, _ResponseRootNode);
                    end;
        ...
        begin
            if((not MobileSetup."Pull Disabled") and (Challenge))then begin
                case SyncLog."Packet Type" of SyncLog."Packet Type"::Data: begin
                    ...
                    if(PullType = PullType::Group)then begin
                        ...
                        DeviceSetup.SetRange(Disabled, false);
                        if DeviceSetup.FindSet(false, false)then repeat ProcessPull(GeneratePullFileName(PullType::Device, DeviceRole, DeviceGroup, DeviceSetup), PullType::Device);
                            until DeviceSetup.Next = 0;
                    end;
                end;
                SyncLog."Packet Type"::DES: SyncMgt.ExportDES;
                SyncLog."Packet Type"::DBS: SyncMgt.ExportDBS;
                end;
            end;
        end;
        /*
    ...
    begin
        ...
        if TableMap.FindSet(false, false)then repeat case TableMap."Use specific push handler" of true: CRUDCyclesCount:=0;
                false: CRUDCyclesCount:=4;
                end;
                ...
                for CRUDCycle:=0 to CRUDCyclesCount do begin
                    ...
                    ConstMgt.CRD_None: if not TableMap."Use specific push handler" then ActiveRec.SetRecordMissingAttributeFilter(ConstMgt.SYN_RecordType);
                    ConstMgt.CRD_Create: ActiveRec.SetRecordAttributeFilter(ConstMgt.SYN_RecordType, ConstMgt.PRS_Create);
                    ConstMgt.CRD_Read: ActiveRec.SetRecordAttributeFilter(ConstMgt.SYN_RecordType, ConstMgt.PRS_Read);
                    ConstMgt.CRD_Update: ActiveRec.SetRecordAttributeFilter(ConstMgt.SYN_RecordType, ConstMgt.PRS_Update);
                    ConstMgt.CRD_Delete: ActiveRec.SetRecordAttributeFilter(ConstMgt.SYN_RecordType, ConstMgt.PRS_Delete);
                    end;
                    ...
                    if(ActiveTableRec.TableRecordsCount <> 0)then begin
                        ...
                        //END ;
                        if ActiveRec.FindRecord then repeat CacheMgt.SetRecNoContext(ActiveRec.GetRecordNo);
                                Clear(CallTriggers);
                        ...
                        case TableMap."Use specific push handler" of true: ProcessPushTableSpecific(RecRef, TableMap);
                        false: GeneralDP.PushProcessGeneral(RecRef, TableMap, CRUDCycle, CallTriggers, ResponseRecRef);
                        end;
    ...
    begin
        ...
        if CacheMgt.FindRPCCache(enum::DYM_PacketDirection::Push)then repeat XMLMgt.AddElement(_DatainNode, CacheMgt.CacheBufferRecID, CacheMgt.CacheBufferData, _DummyNode);
            until CacheMgt.NextRPCCache = 0;
        XMLMgt.AddElement(NodeOut, ConstMgt.RPC_DataOut, '', _DataoutNode);
        if CacheMgt.FindRPCCache(enum::DYM_PacketDirection::Pull)then repeat XMLMgt.AddElement(_DataoutNode, CacheMgt.CacheBufferRecID, CacheMgt.CacheBufferData, _DummyNode);
            until CacheMgt.NextRPCCache = 0;
    end;
    ...
    begin
        ...
        TableMap.SetRange(Disabled, false);
        if TableMap.FindSet(false, false)then repeat Clear(DummyParentTableMap);
                Clear(DummyRecRef);
                ResponseRecRef.Open(TableMap."Table No.", true);
                case TableMap."Use specific pull handler" of true: ProcessPullTableSpecific(DummyParentTableMap, DummyRecRef, TableMap, ResponseRecRef, enum::DYM_PullType::Device);
                false: GeneralDP.PullProcessGeneral(DummyRecRef, ResponseRecRef, TableMap, false);
                end;
    ...
    begin
        case CacheMgt.CheckXMLCache of false: begin
    ...
    begin
        ...
        TableMap.SetRange(Disabled, false);
        case PullType of PullType::Group: TableMap.SetRange("Pull Type", TableMap."Pull Type"::Group);
        PullType::Device: TableMap.SetRange("Pull Type", TableMap."Pull Type"::Device);
        end;
        if TableMap.FindSet(false, false)then repeat ProcessPullTable(DummyParentTableMap, DummyRecRef, TableMap, RootNode, PullType);
            until TableMap.Next = 0;
    ...
    begin
        ...
        RecRef.Open(TableMap."Table No.", true);
        case TableMap."Use specific pull handler" of true: ProcessPullTableSpecific(ParentTableMap, ParentTableData, TableMap, RecRef, PullType);
        false: GeneralDP.PullProcessGeneral(ParentTableData, RecRef, TableMap, false);
        end;
        ...
        case PullType of PullType::Group: ChildTableMap.SetRange("Pull Type", ChildTableMap."Pull Type"::Group);
        PullType::Device: ChildTableMap.SetRange("Pull Type", ChildTableMap."Pull Type"::Device);
        end;
        if ChildTableMap.FindSet(false, false)then repeat ProcessPullTable(TableMap, RecRef, ChildTableMap, XMLRoot, PullType);
    ...
    begin
        ...
        RecRef.Reset();
        if(TableMap.Key = 0)then GeneralDP.PullProcessAutoSelectKey(RecRef, TableMap)
        else
    ...
    procedure GeneratePullFileName(PullType: enum DYM_PullType; var DeviceRole: Record DYM_DeviceRole; var DeviceGroup: Record DYM_DeviceGroup; var DeviceSetup: Record DYM_DeviceSetup)Result: Text[250]var
        GUID: Text[250];
        Extension: Text[250];
    ...
    begin
        ...
        case SettingsMgt.GetSetting(ConstMgt.BOS_PacketStorageType())of ConstMgt.BOS_PacketStorageType_FileSystem(): Begin
            ...
            if(Result <> '')then begin
                ...
                case PullType of PullType::Group: Result+=StrSubstNo(RoleFileNamePrefix, Format(PullType), DeviceRole.Code, CompanyName, DeviceGroup.Code);
                PullType::Device: Result+=StrSubstNo(DeviceFileNamePrefix, Format(PullType), DeviceRole.Code, CompanyName, DeviceGroup.Code, DeviceSetup.Code);
                end;
