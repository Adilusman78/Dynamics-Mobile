codeunit 84036 DYM_SessionDataStoreManagement
{
    SingleInstance = true;

    procedure GetValue(_Key: Text)Result: Text begin
        clear(Result);
    ...
    procedure CheckKey(_Key: text): Boolean begin
    ...
    procedure GetInteger(_Key: Text)Result: Integer begin
        Clear(Result);
        if(KeyValueStore.ContainsKey(_Key))then exit(lldp.Text2Integer(GetValue(_Key)));
    end;
    procedure PopInteger(_Key: Text)Result: Integer begin
        clear(Result);
        if(KeyValueStore.ContainsKey(_Key))then begin
    ...
    var KeyValueStore: Dictionary of[Text, Text];
