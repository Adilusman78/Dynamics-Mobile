codeunit 70145 DYM_SessionManagement
{
    trigger OnRun()
    begin
    end;
    ...
    var
        Text001: Label 'Do you want to delete the Session Value?';
    ...
    local procedure GetDateTimeOffset() Result: Text
    var
        DateTimeFilter: Text;
        DateTimeOffset: Date;
        Offset: Integer;
    ...
    local procedure GetDeliveryPersonCode() Result: Text
    begin
    ...
    procedure LookUpSessionValues(var SessionValueCode: Code[10]) Result: Code[10]
    var
    ...
    begin
        ...
        if (Result <> '') then
            SessionValueCode := Result
        else if (SessionValueCode <> '') then if Confirm(Text001, false) then Clear(SessionValueCode);
    end;

    procedure GetSessionValue(SessionValueCode: Code[10]) Result: Text[250]
    var
        Salesperson: Record "Salesperson/Purchaser";
    begin
        Clear(Result);
        CacheMgt.GetSetup(MobileSetup, DeviceRole, DeviceGroup, DeviceSetup, SLE, SetupRead);
        case SessionValueCode of
            ConstMgt.SES_DRC:
                exit(DeviceRole.Code);
            ConstMgt.SES_DGC:
                exit(DeviceGroup.Code);
            ConstMgt.SES_DSC:
                exit(DeviceSetup.Code);
            ConstMgt.SES_SPC:
                exit(DeviceSetup."Salesperson Code");
            ConstMgt.SES_SPN:
                if Salesperson.Get(DeviceSetup."Salesperson Code") then
                    exit(Salesperson.Name);
            ConstMgt.SES_DML:
                exit(DeviceSetup."Mobile Location");
            ConstMgt.SES_TDY:
                exit(Format(Today));
            ConstMgt.SES_WDT:
            ...
            ConstMgt.SES_TIM:
                exit(Format(Time));
            ConstMgt.SES_BLC:
                exit(SettingsMgt.GetSetting(ConstMgt.BOS_BaseLocationFilter));
            ConstMgt.SES_DTO():
                exit(GetDateTimeOffset());
            ConstMgt.SES_DPC():
                exit(GetDeliveryPersonCode());
            ConstMgt.SES_SOH():
                exit(GetSalesOderHistoryDays());
        end;
    end;

    procedure GetSessionValueDescription(SessionValueCode: Code[10]) Result: Text[30]
    var
        SessionValues: Record DYM_SessionValues;
    begin
        Clear(Result);
    ...
    procedure ApplySessionValuesCodes(InputString: Text[250]) Result: Text[250]
    var
        i: Integer;
    begin
        Clear(Result);
    ...
    procedure ApplySessionValuesToFilter(var FilterString: Text[250]) Result: Text[250]
    var
        SessionValues: Record DYM_SessionValues;
    begin
        Clear(Result);
        Result := FilterString;
        SessionValues.Reset;
        if SessionValues.FindSet(false, false) then
            repeat
                Result := LowLevelDP.ReplaceSingleStr(Result, ConstMgt.SES_Prefix + SessionValues.Code, GetSessionValue(SessionValues.Code));
    ...
    begin
        ...
        if SessionValues.FindSet(false, false) then
            repeat
                SessionValuesHint := SessionValuesHint + StrSubstNo(Text003, SessionValues.Code, SessionValues.Description);
            until SessionValues.Next = 0;
        Message(SessionValuesHint);
    end;
    #endregion 
    #region Context
    procedure GetContextValue(TableName: Text[30]; "Key": Text[250]) Result: Text[250]
    var
    ...
    procedure ApplyContextValuesToFilter(var FilterString: Text[250]) Result: Text[250]
    var
    ...
    #endregion 
    #region Settings
    procedure ApplySettingValuesToFilter(var FilterString: Text[250]) Result: Text[250]
    var
        ContextFilterFound: Boolean;
        ContextVariablePos: Integer;
        DelimiterPos: Integer;
