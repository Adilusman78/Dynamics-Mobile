codeunit 70158 DYM_TrackAssistManagement
{
    var
    ...
    begin
        ...
        if _Positive then begin
            ...
            if ((_SourceType = DATABASE::"Item Journal Line") and (_SourceSubType = ItemJournal."Entry Type"::Transfer.AsInteger())) then
                ReservStatus := ReservStatus::Prospect
            else
            ...
            if ILE.FindSet(false, false) then
                repeat
                    if (TrackingSeesReservations) then ILE.CalcFields("Reserved Quantity");
                    ILEBuffer.Reset;
                    ...
                    case TrackingSeesReservations of
                        true:
                            ILEBuffer."Remaining Quantity" += (ILE."Remaining Quantity" - ILE."Reserved Quantity");
                        false:
                            ILEBuffer."Remaining Quantity" += (ILE."Remaining Quantity");
                    end;
                    ILEBuffer.Modify();
                until ILE.Next = 0;
            ILEBuffer.Reset;
            ...
            if ILEBuffer.FindSet(false, false) then
                repeat
                    if (QtyToTrack > ILEBuffer."Remaining Quantity") then begin
    ...
    begin
        ...
        if TrackingSpecification.FindSet(false, false) then
            repeat
                ReservEntry.Reset;
    ...
    begin
        ...
        ReservEntry.MODIFYALL("Qty. to Handle (Base)", 0);
        ReservEntry.MODIFYALL("Qty. to Invoice (Base)", 0);
        Message('DeleteTrackingQtytoHandlePL fired [%1]', ReservEntry.Count)
    end;
    ...
    local procedure FilterReservEntry(_Rec: Variant; var _ReservEntry: Record "Reservation Entry"): Boolean
    var
        RecRef: RecordRef;
    ...
    procedure CheckIfItemTrackingIsEnabled(_ItemNo: Code[20]) Result: boolean
    var
        Item: Record Item;
    ...
    procedure CheckReservEntryExist(_Rec: Variant): Boolean
    var
        ReservEntry: Record "Reservation Entry";
    begin
        ReservEntry.Reset();
    ...
    procedure GetReservEntryLot(_Rec: Variant): Code[20]
    var
        ReservEntry: Record "Reservation Entry";
    begin
        Clear(ReservEntry);
    ...
    procedure GetReservEntryExpirationDate(_Rec: Variant): Date
    var
        ReservEntry: Record "Reservation Entry";
    begin
        Clear(ReservEntry);
    ...
    procedure GetLotExpirationDate(ItemNo: Code[20]; LocationCode: Code[10]; LotNo: Code[20]) Result: Date
    var
        ILE: Record "Item Ledger Entry";
    begin
        Clear(Result);
