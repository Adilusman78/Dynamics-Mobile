codeunit 70100 "DYM_WH_DataProcess"
{
    ...
    begin
        ...
        if ar_SalesHead.FindRecord then
            repeat
                DataLogMgt.LogActiveRecOp(ar_SalesHead.GetTableName(), ar_SalesHead.ActiveRecPK2Text());
                if SalesHeader.Get(SalesHeader."Document Type"::Order, ar_SalesHead.TextField(F_SalesHead_No)) then begin
                    ...
                    if ar_SalesLines.FindRecord then
                        repeat
                            DataLogMgt.LogActiveRecOp(ar_SalesLines.GetTableName(), ar_SalesLines.ActiveRecPK2Text());
    ...
    begin
        ...
        if ar_PurchHead.FindRecord then
            repeat
                DataLogMgt.LogActiveRecOp(ar_PurchHead.GetTableName(), ar_PurchHead.ActiveRecPK2Text());
                if PurchHeader.Get(PurchHeader."Document Type"::Order, ar_PurchHead.TextField(F_PurchHead_No)) then begin
                    ...
                    */
                    ...
                    if ar_PurchLines.FindRecord then
                        repeat
                            DataLogMgt.LogActiveRecOp(ar_PurchLines.GetTableName(), ar_PurchLines.ActiveRecPK2Text());
    ...
    begin
        ...
        if ar_WhseActLine.FindRecord then
            repeat
                DataLogMgt.LogActiveRecOp(ar_WhseActLine.GetTableName(), ar_WhseActLine.ActiveRecPK2Text());
                case (ar_WhseActLine.IntegerField(F_WhseActLine_ActivityType)) of
                    12: //Create Manual Move
                        begin
                    ...
                    13: //Movement request
                    ...
                    else begin //Default handler
                        Clear(MasterOutstandingQty);
                        case ar_WhseActLine.BooleanField(F_WhseActLine_Breakbulk) of
                            false:
                                MasterWhseActLine.Get(ar_WhseActLine.OptionField(F_WhseActLine_ActivityType, '5767,1'), ar_WhseActLine.TextField(F_WhseActHead_No), ar_WhseActLine.IntegerField(F_WhseActLine_LineNo));
                            true:
                                MasterWhseActLine.Get(ar_WhseActLine.OptionField(F_WhseActLine_ActivityType, '5767,1'), ar_WhseActLine.TextField(F_WhseActLine_No), ar_WhseActLine.IntegerField(F_WhseActLine_ParentLineNo));
                        end;
                        ...
                        if (MasterWhseActLine."Activity Type" <> MasterWhseActLine."Activity Type"::Movement) then begin
                            ...
                            case MasterWhseActLine."Activity Type" of
                                MasterWhseActLine."Activity Type"::"Put-away":
                                    begin
                                ...
                                MasterWhseActLine."Activity Type"::Pick:
                                    begin
    ...
    begin
        ...
        if ar_TransHead.FindRecord then
            repeat //Check for new transfer. Not using filter for backward compatibility. Regular transfers are not sending "newTransfer" tag
                if not (ar_TransHead.BooleanField(F_TransHead_IsNewTransfer)) then begin
                    ...
                    if TransHeader.Get(ar_TransHead.TextField(F_TransHead_No)) then begin
                        ...
                        if ar_TransLines.FindRecord then
                            repeat
                                DataLogMgt.LogActiveRecOp(ar_TransLines.GetTableName(), ar_TransLines.ActiveRecPK2Text());
                                if TransLines.Get(ar_TransLines.TextField(F_TransLine_DocumentNo), ar_TransLines.IntegerField(F_TransLine_LineNo)) then begin
                                    ...
                                    case IsInbound of
                                        0:
                                            if (ar_TransLines.DecimalField(F_TransLine_QuantityToShip) <> 0) then begin
                                        ...
                                        1:
                                            if (ar_TransLines.DecimalField(F_TransLine_QtyToReceive) <> 0) then begin
                        ...
                        IF TransLines.findset(false, false) then
                            repeat
                                case IsInbound of
                                    0:
                                        If (TransLines."Outstanding Quantity" <> TransLines."Qty. to Ship") then begin
                                            FullyProcessed := false;
                                        end;
                                    1:
                                        If (TransLines."Qty. in Transit" <> TransLines."Qty. to Receive") then begin
                                            FullyProcessed := false;
                                        end;
                                end;
                            ...
                            false:
                                begin
                                    case (IsInbound) of
                                        0:
                                            begin
                                                TransHeader.VALIDATE(DYM_MobileShipStatus, TransHeader.DYM_MobileShipStatus::"Partially Processed")
                                            end;
                                        1:
                                            begin
                        ...
                        if (ar_TransHead.BooleanField(F_TransHead_IsFinished)) then begin
                            case (ar_TransHead.TextField(F_TransHead_IsInbound)) of
                                O_TransHead_IsInbound_Outbound:
                                    TransHeader.VALIDATE(DYM_MobileShipStatus, TransHeader.DYM_MobileShipStatus::Processed);
                                O_TransHead_IsInbound_Inbound:
                                    TransHeader.VALIDATE(DYM_MobileReceiveStatus, TransHeader.DYM_MobileReceiveStatus::Processed);
                            end;
                            TransHeader.Modify();
                        end;
    ...
    begin
        ...
        if ar_WhseShptHead.FindRecord then
            repeat
                DataLogMgt.LogActiveRecOp(ar_WhseShptHead.GetTableName(), ar_WhseShptHead.ActiveRecPK2Text());
                ...
                if ar_WhseShptLines.FindRecord then
                    repeat
                        DataLogMgt.LogActiveRecOp(ar_WhseShptLines.GetTableName(), ar_WhseShptLines.ActiveRecPK2Text());
    ...
    begin
        ...
        if ar_WhseRcptHead.FindRecord then
            repeat
                WhseReceiptHeader.Get(ar_WhseRcptHead.TextField(F_WhseRcptHead_No));
                ...
                if ar_WhseRcptLines.FindRecord then
                    repeat
                        WhseReceiptLines.Get(ar_WhseRcptLines.TextField(F_WhseRcptLine_No), ar_WhseRcptLines.IntegerField(F_WhseRcptLine_LineNo));
    ...
    begin
        ...
        if ar_WhseJnlLine.FindRecord then
            repeat
                DataLogMgt.LogActiveRecOp(ar_WhseJnlLine.GetTableName(), ar_WhseJnlLine.ActiveRecPK2Text());
 ...
 ar_WhseJnlLine.TextField(F_StockTakeLine_BinCode), ar_WhseJnlLine.DecimalField(F_StockTakeLine_Qty), ar_WhseJnlLine.TextField(F_StockTakeLine_SerialNo), ar_WhseJnlLine.TextField(F_StockTakeLine_LotNo), ar_WhseJnlLine.TextField(F_StockTakeLine_PackageNo), ar_WhseJnlLine.DateField(F_StockTakeLine_ExpirationDate));
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if ar_SalesLines.FindRecord then
                repeat
                    SalesHeader.Get(ar_SalesLines.OptionField(F_SalesLine_DocumentType, O_SalesLine_DocumentType), ar_SalesLines.TextField(F_SalesLine_DocumentNo));
                    ...
                    if TrackAssistMgt.CheckIfItemTrackingIsEnabled(Item."No.") then begin
                        ...
                        if ar_TrackSpec.FindRecord then
                            repeat
                                Clear(UOMAssistMgt);
                                ...
                                case ar_TrackSpec.TextField(F_TrackSpec_EntryNo) = '' of
                                    true:
                                        begin
        ...
        if not IsHandled then begin
            ...
            if ar_PurchLines.FindRecord then
                repeat
                    PurchHeader.Get(ar_PurchLines.OptionField(F_PurchLine_DocumentType, O_PurchLine_DocumentType), ar_PurchLines.TextField(F_PurchLine_DocumentNo));
        ...
        if not IsHandled then begin
            ...
            if ar_WhseRcptLines.FindRecord then
                repeat
                    WhseReceiptLine.Get(ar_WhseRcptLines.TextField(F_WhseRcptLine_No), ar_WhseRcptLines.TextField(F_WhseRcptLine_LineNo));
                    case WhseReceiptLine."Source Type" of
                        DATABASE::"Purchase Line":
                            PurchLine.Get(WhseReceiptLine."Source Subtype", WhseReceiptLine."Source No.", WhseReceiptLine."Source Line No.");
                    ...
                    if TrackAssistMgt.CheckIfItemTrackingIsEnabled(Item."No.") then begin
                        ...
                        case WhseReceiptLine."Source Type" of
                            DATABASE::"Purchase Line":
                                ar_TrackSpec.SetFieldFilter(F_TrackSpec_SourceRefNo, LowLevelDP.Integer2Text(WhseReceiptLine."Source Line No."));
                            DATABASE::"Transfer Line":
                                ar_TrackSpec.SetFieldFilter(F_TrackSpec_SourceProdOrderLine, LowLevelDP.Integer2Text(WhseReceiptLine."Source Line No."));
                        end;
                        //ar_TrackSpec.SetFieldFilter(F_TrackSpec_QuantityBase, '<>0');
                        if ar_TrackSpec.FindRecord then
                            repeat
        ...
        if not IsHandled then begin
            ...
            if ar_WhseShptLines.FindRecord then
                repeat
                    if not TrackingUpdated then begin
                        //TODO:Tracking handling
                    ...
                    if TrackAssistMgt.CheckIfItemTrackingIsEnabled(Item."No.") then begin
                        ...
                        if ar_TrackSpec.FindRecord then
                            repeat
                                DataLogMgt.LogActiveRecOp(ar_TrackSpec.GetTableName(), ar_TrackSpec.ActiveRecPK2Text());
                                case (WhseShipmentLine."Qty. Picked (Base)" = 0) of
                                    true:
                                        begin
                                            Clear(UOmAssistMgt);
                                            Clear(TrackingQty);
        ...
        if not IsHandled then begin
            ...
            if ar_WhseTransferLines.FindRecord then
                repeat
                    if not (ar_WhseTransferLines.BooleanField(F_TransLine_IsNewTransfer)) then begin
        ...
        if not IsHandled then begin
            ...
            if ar_WhseTransferLines.FindRecord then
                repeat
                    if (ar_WhseTransferLines.BooleanField(F_TransLine_IsNewTransfer)) then begin
    ...
    begin
        ...
        if ar_ItemJnlLine.FindRecord then
            repeat
                clear(fLineNo);
                clear(fItemNo);
                clear(fUOMCode);
    ...
    begin
        ...
        if (ar_Barcode.FindRecord()) then
            repeat
                ItemNo := ar_Barcode.TextField(F_CrossRef_ItemNo);
    ...
    begin
        ...
        if ar_ProdOrder.FindRecord() then
            repeat
                SettingsMgt.TestSetting(ConstMgt.BOS_Prod_Output_JournalTemplate());
                ...
                if ar_ProdOrderLine.FindRecord() then
                    repeat
                        clear(pLotNo);
            ...
            repeat
                ...
                if ar_ProdOrderComp.FindRecord() then
                    repeat
                        ...
                        //Item.Get(fItemNo);
                        case fIsCustomAdded of
                            false:
                                ProdOrderComp.Get(fProdOrderStatus, fProdOrderNo, fProdOrderLineNo, fLineNo);
                            true:
                                Clear(fLineNo);
                        end;
                        if (TrackAssistMgt.CheckIfItemTrackingIsEnabled(fItemNo)) then begin
                            ...
                            if ar_ReservEntry.FindRecord() then
                                repeat
                                    pLotNo := ar_ReservEntry.TextField(F_TrackSpec_LotNo);
    ...
    begin
        ...
        if ar_Bin.FindRecord() then
            repeat
                Bin.Get(ar_Bin.TextField(F_Bin_LocationCode), ar_Bin.TextField(F_Bin_Code));
    ...
    begin
        ...
        if ar_WhseJnlLine.FindRecord() then
            repeat
                JournalLocationCode := ar_WhseJnlLine.TextField(F_WhseJnlLine_LocationCode);
                WhseJnlBatch.Get(JournalTemplateName, JournalBatchName, JournalLocationCode);
                WhseJnlBatch.TestField("No. Series", '');
                case ar_WhseJnlLine.TextField(F_WhseJnlLine_CorrectionType) of
                    O_WhseJnlLine_CorrectionType_Negative:
                        EntryType := 0;
                    O_WhseJnlLine_CorrectionType_Positive:
                        EntryType := 1;
                    else
    ...
    begin
        ...
        IF ar_TransferHeader.FindRecord THEN
            REPEAT
                if (ar_TransferHeader.BooleanField(F_TransHead_IsNewTransfer)) then begin
                    TransferHeader.INIT;
                    ...
                    IF ar_TransferLine.FindRecord THEN
                        REPEAT
                            NextLineNo += 10000;
                            TransferLine.INIT;
