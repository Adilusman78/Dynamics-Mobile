codeunit 70115 DYM_DocumentTraceManagement
{
    var
    ...
    begin
        ...
        if DataLog.FindSet() then
            repeat
                if (SyncLog."Entry No." <> DataLog."Sync Log Entry No.") then
                    if SyncLog.Get(DataLog."Sync Log Entry No.") then
                        if SyncLogStatus2TraceStatus(SyncLog.Status).AsInteger() > SyncLogTraceStatus.AsInteger() then begin
    ...
    begin
        ...
        if PostProcessLog.FindSet() then
            repeat
                if PostProcessLogStatus2TraceStatus(PostProcessLog.Status).AsInteger() > PostProcessLogTraceStatus.AsInteger() then begin
    ...
    local procedure SyncLogStatus2TraceStatus(SyncLogStatus: enum DYM_PacketStatus) Result: enum DYM_DocumentTraceStatus
    begin
        clear(Result);
        Case SyncLogStatus of
            SyncLogStatus::Failed, SyncLogStatus::Error:
                Result := Result::Failed;
            SyncLogStatus::InProgress:
    ...
    local procedure PostProcessLogStatus2TraceStatus(PostProcessLogStatus: enum DYM_PostProcessPacketStatus) Result: enum DYM_DocumentTraceStatus
    begin
        clear(Result);
        Case PostProcessLogStatus of
            PostProcessLogStatus::Failed, PostProcessLogStatus::Error:
                Result := Result::Failed;
            PostProcessLogStatus::Pending, PostProcessLogStatus::InProgress:
                Result := Result::Pending;
            PostProcessLogStatus::Success:
                Result := Result::Success;
        End;
    end;

    local procedure getRecRefDocumentNo(RecRef: RecordRef) Result: Code[20]
    var
