codeunit 70105 DYM_BLOBStoreManagement
{
    var
    ...
    procedure parseBLOBPath(_FullBLOBPath: Text) Result: Text
    begin
    ...
    procedure findBLOBStoreEntry(_DeviceSetupCode: Code[100]; _Identifier: Text) Result: Integer
    var
    ...
    begin
        ...
        if BLOBStore.FindLast() then
            exit(BLOBStore."Entry No.")
        else
            error(Text002, _DeviceSetupCode, _Identifier);
    end;

    procedure packetType2FileExtension(_PacketType: Text) Result: Text
    begin
        Clear(Result);
        case _PacketType of
            'image':
    ...
    procedure applyFileExtension(_Identifier: Text; _PacketType: Text) Result: Text
    begin
