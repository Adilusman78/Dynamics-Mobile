codeunit 70149 DYM_SL_DataProcess
{
    ...
    begin
        ...
        if ar_SalesHeader.FindRecord then
            repeat
                CacheMgt.SetStateDescription(StrSubstNo(ConstMgt.MSG_0009, ar_SalesHeader.GetTableName, ar_SalesHeader.ActiveRecPK2Text));
                ...
                if (Customer.Blocked <> Customer.Blocked::" ") then
                    if SettingsMgt.CheckSetting(ConstMgt.BOS_ProcessBlockedCustomers) then begin
                        BlockCust := Customer.Blocked;
                        Customer.Blocked := Customer.Blocked::" ";
                        Customer.Modify;
                    end;
                SalesHeader.Init;
                case ar_SalesHeader.IntegerField(F_SalesHeader_DocumentType) of
                    0: //Order
                        begin
                ...
                if not IsDelivery then begin
                    ...
                    if (ar_SalesHeader.DateField(F_SalesHeader_OrderDate) <> 0D) then
                        SalesHeader.Validate("Order Date", ar_SalesHeader.DateField(F_SalesHeader_OrderDate))
                    else
                        SalesHeader.Validate("Order Date", ar_SalesHeader.DateField(F_SalesHeader_DocumentDate));
                    if (ar_SalesHeader.DateField(F_SalesHeader_PostingDate) <> 0D) then
                        SalesHeader.Validate("Posting Date", ar_SalesHeader.DateField(F_SalesHeader_PostingDate))
                    else
                        ...
                        2:
                            begin //Return Order
                                if (ar_SalesHeader.TextField(F_SalesHeader_ReturnReason) <> '') then begin
                                    ...
                                    case ReturnReason.DYM_ReturnReasonType of
                                        DYM_ReturnReasonType::Inventory:
                                            begin
                ...
                if ar_SalesLines.FindRecord then
                    repeat
                        ...
                        if Item.Blocked then
                            if SettingsMgt.CheckSetting(ConstMgt.BOS_ProcessBlockedItems) then begin
                                Item.Blocked := false;
                                Item.Modify;
                                BlockItem := true;
                            end;
                        ...
                        if (ar_SalesLines.TextField(F_SalesLines_LocationCode) <> '') then
                            SalesLines.Validate("Location Code", ar_SalesLines.TextField(F_SalesLines_LocationCode))
                        else
                ...
                if SalesLines.FindSet(false, false) then
                    repeat
                        ...
                        if (SalesLines."Inv. Discount Amount" = 0) and (SalesLines."Line Discount Amount" = 0) and (not SalesHeader."Prices Including VAT") then
                            ICAS."Unit Cost" := SalesLines."Unit Price"
                        else if SalesHeader."Prices Including VAT" then
                            ICAS."Unit Cost" := Round((SalesLines."Line Amount" - SalesLines."Inv. Discount Amount") / SalesLines.Quantity / (1 + SalesLines."VAT %" / 100), Currency."Unit-Amount Rounding Precision")
                        else
    ...
    begin
        ...
        if ar_Customer.FindRecord then
            repeat
                DataLogMgt.LogActiveRecOp(ar_Customer.GetTableName, ar_Customer.ActiveRecPK2Text);
                clear(fCusmtomerNo);
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if ar_SalesLines.FindRecord then
                repeat
                    SalesLine.GetBySystemId(RecordLinkMgt.GetRecordLinkRecId(T_SalesLines, ar_SalesLines.TextField(ConstMgt.SYS_DMSRowId())));
                    ar_SalesHeader.ReadTable(T_SalesHeader);
                    ...
                    if TrackAssistMgt.CheckIfItemTrackingIsEnabled(Item."No.") then begin
                        ...
                        if ar_TrackingSpecification.FindRecord then
                            repeat
                                DataLogMgt.LogActiveRecOp(ar_TrackingSpecification.GetTableName, ar_TrackingSpecification.ActiveRecPK2Text);
                                ReservEntry.Reset;
                        ...
                        ar_TrackingSpecification.SetFieldFilter(F_TrackingSpecification_SourceRefNo, ar_SalesLines.TextField(F_SalesLines_LineNo));
                        if ar_TrackingSpecification.FindRecord then
                            if (ar_TrackingSpecification.IntegerField(F_SalesLines_ReturnInventTransId) <> 0) then begin
        ...
        if not IsHandled then begin
            ...
            if ar_TransferLine.FindRecord then
                repeat
                    TransferLine.GetBySystemId(RecordLinkMgt.GetRecordLinkRecId(T_TransferLine, ar_TransferLine.TextField(ConstMgt.SYS_DMSRowId())));
    ...
    begin
        ...
        if ar_PaymentHeader.FindRecord then
            repeat //Generate unapplied amount line
                CLEAR(PaidAmount);
                CLEAR(AppliedAmount);
                ...
                if ar_PaymentLine.FindRecord() THEN
                    REPEAT
                        AppliedAmount := AppliedAmount + ar_PaymentLine.DecimalField(F_PaymentLine_amount);
                ...
                if ar_PaymentLine.FindRecord then
                    repeat
                        DataLogMgt.LogActiveRecOp(ar_PaymentLine.GetTableName, ar_PaymentLine.GetActiveRecContext);
    ...
    begin
        ...
        if ar_TransHeader.FindRecord then
            repeat
                DataLogMgt.LogActiveRecOp(ar_TransHeader.GetTableName, ar_TransHeader.GetActiveRecContext);
                TransferHeader.Init;
                ...
                if ar_TransLine.FindRecord then
                    repeat //z
    ...
    begin
        ...
        if ar_Snapshot.FindRecord() then
            repeat
                PostProcessMgt.LogPostProcessOperation(RecRef, enum::DYM_PostProcessOperationType::Custom, ConstMgt.PPP_ProcessSnapshots(), 0, false, BLOBStoreMgt.parseBLOBPath(ar_Snapshot.TextField(F_Snapshot_ImageName)));
    ...
    begin
        ...
        if ar_SalesHead.FindRecord then
            repeat
                if SalesHeader.Get(SalesHeader."Document Type"::Order, ar_SalesHead.TextField(F_SalesHeader_SourceDocumentNo)) then begin
                    ...
                    if (SalesLines.FindSet()) then
                        repeat
                            if (SalesLines.Quantity <> (SalesLines."Qty. to Ship" + SalesLines."Quantity Shipped")) then Clear(FullyProcessed);
                        until SalesLines.Next() = 0;
    ...
    begin
        ...
        if ar_CustomerActivity.FindRecord then
            repeat
                Clear(DeviceActivity);
                DeviceActivity.Init();
    ...
    begin
        ...
        if (CustomerRecRef.FindSet()) then
            repeat
                FieldRef := CustomerRecRef.Field(1);
                ...
                if (SalesInvoiceHeader.FindSet()) then
                    repeat
                        ...
                        if not IsHandled then
                            if (SettingsMgt.CheckSetting(ConstMgt.BOS_SkipCreditedReturnInvoices())) then begin
                        ...
                        if (not SkipDocument) then begin
                            ...
                            if not (SalesInvoiceLineBuffer.IsEmpty()) then
                                if not SalesInvoiceHeaderBuffer.Get(SalesInvoiceHeader."No.") then begin
    ...
    procedure L_T_SalesHeader(): text
    begin
        exit(T_SalesHeader);
    end;
    #endregion 
