codeunit 70165 DYM_XMLManagement
{
    trigger OnRun()
    begin
    end;
    ...
    procedure GetNodeValue(_Node: XmlNode; _NodeName: Text[250]; Nested: Boolean) Result: Text[250]
    var
        ResultNode: XmlNode;
    begin
        Clear(Result);
    ...
    procedure TryGetNodeValue(Node: XmlNode; Name: Text; Nested: Boolean) Result: Text[250]
    var
    ...
    begin
        case (value = '') of
            true:
                NewChildNode := XmlElement.Create(NodeName, NamespaceURI).AsXmlNode();
            false:
                NewChildNode := XmlElement.Create(NodeName, NamespaceURI, Value).AsXmlNode();
        end;
    ...
    begin
        case (value = '') of
            true:
                NewChildNode := XmlElement.Create(NodeName, '').AsXmlNode();
            false:
                NewChildNode := XmlElement.Create(NodeName, '', Value).AsXmlNode();
        end;
        node.AsXmlElement().Add(NewChildNode);
        CreatedXMLNode := NewChildNode;
    end;
    ...
    procedure AddBinary(var Node: XmlNode; Name: Text[250]; var BlobData: InStream; var CreatedNode: XmlNode) Status: Integer
    var
    ...
    procedure GetXPathFilter("Filter": Text[250]; Value: Text[250]): Text[250]
    begin
        exit('[@' + Filter + '="' + Value + '"]');
    end;

    procedure GetXPathMissingFilter("Filter": Text[250]): Text[250]
    begin
        exit('[not(@' + Filter + ')]');
    end;
}
