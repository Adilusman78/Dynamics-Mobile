codeunit 70133 DYM_MobileDataProcess
{
    ...
    begin
        ...
        if FieldMap.FindSet(false, false) then begin
            repeat
                if (FieldMap."Field No." = 0) then begin
                    PushBuffer.Init;
                    ...
                    if (FieldMap.IsAutoIncrementPush) then begin
                        ...
                        case SessionBased of
                            true:
                                TempText := SessionValue;
                            false:
                                case FldType of
                                    ConstMgt.FDT_BLOB:
                                        ActiveRec.BLOB2BLOBFieldRef(FieldMap."Mobile Field", FldRef);
                                    ConstMgt.FDT_Media:
                                        ActiveRec.BLOB2MediaFieldRef(FieldMap."Mobile Field", FldRef);
                                    else
            ...
            PushBuffer.Reset;
            if PushBuffer.FindSet(false, false) then
                repeat
                    CacheMgt.SetCacheBufferEntry(TableMap."Table No.", 0, '', 0, 0, PushBuffer."Mobile Field", RecRef.GetPosition, enum::DYM_PacketDirection::Push, PushBuffer.Data);
    ...
    procedure GetAIEntryNo(): Integer
    begin
        exit(AIEntryNo);
    end;
