codeunit 70134 DYM_PacketManagement
{
    var
    ...
    begin
        ...
        Case SettingsMgt.CheckGlobalSetting(ConstMgt.BOS_MaxJobProcessCalls()) of
            false:
                MaxJobProcessCalls := 1;
            true:
                MaxJobProcessCalls := LowLevelDP.Text2Integer(SettingsMgt.GetGlobalSetting(ConstMgt.BOS_MaxJobProcessCalls()));
        end;
    ...
    procedure GetMaxJobRunDuration(): Duration
    begin
    ...
    begin
        ...
        if (params.Contains(Param_All)) OR (params.Contains(Param_Load)) then begin
            ...
            if SyncLog.FindSet(false, false) then
                repeat
        ...
        if (params.Contains(Param_All)) OR (params.Contains(Param_Process) OR (params.Contains(Param_Process_Internal)) OR (params.Contains(Param_Process_Regular))) then begin
            ...
            case Retry of
                false:
                    SyncLog.SetRange(Status, SyncLog.Status::Parsed);
                true:
                    begin
            ...
            if SyncLog.FindSet(false, false) then begin
                ...
                repeat
                    ...
                    if PostProcessLog.FindSet(false, false) then
                        repeat
                            CacheMgt.SetContextPPE(PostProcessLog."Entry No.");
        ...
        if (params.Contains(Param_All)) OR (params.Contains(Param_LoadBLOB)) then begin
            ...
            if BLOBStoreEntry.FindSet(false, false) then
                repeat
                    CacheMgt.SetContextBSE(BLOBStoreEntry."Entry No.");
    ...
    begin
        ...
        case Result of
            ...
            true:
                begin
                    case SyncLog2.Direction of
                        synclog2.Direction::Push:
                            SyncLog2.Status := SyncLog2.Status::Loaded;
                        SyncLog2.Direction::Pull:
                            case synclog2.Internal of
                                false:
                                    SyncLog2.Status := SyncLog2.Status::Success;
                                true:
                                    SyncLog2.Status := SyncLog2.Status::Pending;
                            end;
                    end;
    ...
    begin
        ...
        case Result of
            false:
                begin
                    case SDSHandler.PP_Flag_Skipped_Pop(PostProcessLog2) of
                        true:
                            begin
    ...
    begin
        ...
        for i := 1 to LowLevelDP.GetSubStrCount(ParamsText) do begin
            ...
            case LowLevelDP.IsCurlyBracketEncapsulated(ParamText) of
                false:
                    Params.Add(ParamText);
                true:
                    DeviceFilterText := CopyStr(ParamText, 2, StrLen(ParamText) - 2);
            end;
        end;
