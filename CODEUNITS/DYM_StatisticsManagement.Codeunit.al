codeunit 70154 DYM_StatisticsManagement
{
    var
    ...
    #region Statistics
    procedure GetStatistics(SyncLog: Record DYM_SyncLog; var DeviceRoleCode: Code[20]; var DeviceGroupCode: Code[20]; var DeviceSetupCode: Code[100]; var RecStats: Record "Dimension Selection Buffer") Result: Boolean
    var
    ...
    procedure ReadPacketData(SyncLog: Record DYM_SyncLog; TableName: Text[30]; var Data: Record DYM_CacheBuffer) Result: Boolean
    var
    ...
    begin
        ...
        if PacketRecords.FindRecord then
            repeat
                Clear(FieldNo);
                RowNo += 1;
                if PacketFields.FindField(TableName) then
                    repeat
                        FieldNo += 1;
                        Data.Init;
                        ...
                        if (StrLen(Str) <= MaxStrLen(Data.Data)) then
                            Data.Data := Str
                        else
    ...
    begin
        ...
        if DataLog.FindSet() then
            repeat
                if not DataLogTableDict.ContainsKey(DataLog."Table No.") then DataLogTableDict.Add(DataLog."Table No.", false);
                GetPostedDocuments(RecStats, AddDataLog, DataLog."Table No.", DataLog.Position);
            until DataLog.Next() = 0;
        AddDataLog.Reset();
        if AddDataLog.FindSet() then
            repeat
                if not DataLogTableDict.ContainsKey(AddDataLog."Table No.") then DataLogTableDict.Add(AddDataLog."Table No.", true);
    ...
    procedure DataLogNavigateTableListSelector(_DataLogTablesList: Dictionary of [Integer, Boolean]; _SourceType: Boolean) Result: Integer
    begin
        clear(Result);
    ...
    procedure GetNavigation(SyncLog: Record DYM_SyncLog; var RecStats: Record "Dimension Selection Buffer"; var AddDataLog: Record DYM_DataLog; FillBuffer: Boolean) Result: Boolean
    var
        DataLog: Record DYM_DataLog;
    begin
        Clear(Result);
        ...
        if DataLog.FindSet(false, false) then
            repeat
                DataLog.SetRange("Table No.", DataLog."Table No.");
    ...
    begin
        if _RecRef.FindSet() then begin
            ...
            repeat
                InsertAddDataLog(_AddDataLog, _RecRef.Number, _RecRef.GetPosition());
    ...
    begin
        ...
        if (RecordsFound) then begin
            ...
            if AddDataLog.FindSet(false, false) then
                repeat
                    RecRef.SetPosition(AddDataLog.Position);
                    RecRef.Mark(true);
                    RecordsFound := true;
    ...
    #endregion 
    #region Checks
    procedure calcStreamChecksum(_inStream: InStream): Text
    var
    ...
    procedure CheckFieldEditable(TableName: Text; FieldName: Text) Result: Boolean
    var
