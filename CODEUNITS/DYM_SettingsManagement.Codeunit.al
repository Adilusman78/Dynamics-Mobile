codeunit 70146 DYM_SettingsManagement
{
    trigger OnRun()
    begin
        CheckOrphanMapping;
    ...
    begin
        ...
        while not Ins.EOS do begin
            ...
            if (StrLen(RecText) = 0) then begin
                TableCounter += 1;
                RecRef.Close;
                case TableCounter of
                    2:
                        RecRef.Open(DATABASE::DYM_DynamicsMobileSetup);
                    3:
                        RecRef.Open(DATABASE::DYM_MobileTableMap);
                    4:
                        RecRef.Open(DATABASE::DYM_MobileFieldMap);
                    5:
                        RecRef.Open(DATABASE::DYM_MobileTableMapFilters);
                    6:
                        RecRef.Open(DATABASE::DYM_DeviceRole);
                    7:
                        RecRef.Open(DATABASE::DYM_DeviceGroup);
                    8:
                        RecRef.Open(DATABASE::DYM_DeviceSetup);
                end;
            end
    ...
    begin
        ...
        if TableMap.FindSet(false, false) then
            repeat
                ...
                if FieldMap.FindSet(false, false) then
                    repeat
                        if FieldMapBuffer.Get(FieldMap."Device Role Code", FieldMap."Table No.", FieldMap."Table Index", FieldMap."Field No.", FieldMap."Field Index") then FieldMapBuffer.Delete;
                    until FieldMap.Next = 0;
    ...
    procedure CheckSetting(SettingCode: Text[50]) Result: Boolean
    var
        SettingsAssignment: Record DYM_SettingsAssignment;
    begin
        Clear(Result);
    ...
    procedure GetSetting(SettingCode: Text[50]) Result: Text[100]
    var
        SettingsAssignment: Record DYM_SettingsAssignment;
    begin
        Clear(Result);
    ...
    procedure CheckGlobalSetting(SettingCode: Text[50]) Result: Boolean
    var
        SettingsAssignment: Record DYM_SettingsAssignment;
    begin
        CLEAR(Result);
    ...
    procedure GetGlobalSetting(SettingCode: Text[50]) Result: Text[100]
    var
        SettingsAssignment: Record DYM_SettingsAssignment;
    begin
        CLEAR(Result);
    ...
    procedure GetGlobalSetting(SettingCode: Text[50]; UseDefaults: Boolean) Result: Text[100]
    var
        ConstMgt: Codeunit DYM_ConstManagement;
    begin
        Clear(Result);
        ...
        if (Result = '') then begin
            case SettingCode of
                ConstMgt.CLD_APIURL():
                    Result := ConstMgt.DSV_cloud_APIURL();
                ConstMgt.CLD_URL_fetchPacketsLink():
                    Result := ConstMgt.DSV_cloud_URL_fetchPacketsLink();
            end;
        end;
    end;
    ...
    procedure GetPublishedSettingsCode(PublishedTo: enum DYM_SettingsPublishType; PublishedPos: Integer): Text[50]
    var
        Settings: Record DYM_Settings;
    ...
    procedure GetSettingsCaptions(CaptionExpr: Text[80]) Result: Text[1024]
    var
    ...
    begin
        ...
        if (SelectedSettingCode <> '') then
            SettingCode := SelectedSettingCode
