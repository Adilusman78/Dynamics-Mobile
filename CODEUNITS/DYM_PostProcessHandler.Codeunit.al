codeunit 70137 DYM_PostProcessHandler
{
    #region Variables
    var
    ...
    begin
        ...
        case PostProcessEntry."Operation Type" of
            PostProcessEntry."Operation Type"::Register:
                begin
                    if ((WhseActivityHeader.Type = WhseActivityHeader.Type::Movement) or (WhseActivityHeader.Type = WhseActivityHeader.Type::"Put-away") or (WhseActivityHeader.Type = WhseActivityHeader.Type::Pick) or (WhseActivityHeader.Type = WhseActivityHeader.Type::"Invt. Movement") or (WhseActivityHeader.Type = WhseActivityHeader.Type::"Invt. Put-away")) then begin
                        ...
                        case WhseActivityHeader.Type of
                            WhseActivityHeader.Type::Pick:
                                if not SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_Pick) then
                                    SetStatusSkipped();
                            WhseActivityHeader.Type::"Put-away":
                                if not SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_PutAway) then
                                    SetStatusSkipped();
                            WhseActivityHeader.Type::Movement:
                                if not SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_Movement) then
                                    SetStatusSkipped();
                            WhseActivityHeader.Type::"Invt. Movement":
                                if not SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_InvMovement) then
                                    SetStatusSkipped();
                            WhseActivityHeader.Type::"Invt. Put-away":
                                if not SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_InvPutAway()) then
                                    SetStatusSkipped();
                        end;
                        Clear(WMSMgt);
            ...
            PostProcessEntry."Operation Type"::Print:
                begin
                    Clear(WhsePrint);
                    //Ext
                    ...
                    case WhseActivityHeader.Type of
                        WhseActivityHeader.Type::Pick:
                            if SettingsMgt.CheckSetting(ConstMgt.BOS_Print_WhsePick) then
                                WhsePrint.PrintPickHeader(WhseActivityHeader)
                            else
                                SetStatusSkipped();
                        WhseActivityHeader.Type::"Put-away":
                            if SettingsMgt.CheckSetting(ConstMgt.BOS_Print_WhsePutAway) then
                                WhsePrint.PrintPutAwayHeader(WhseActivityHeader)
                            else
                                SetStatusSkipped();
                        WhseActivityHeader.Type::Movement:
                            if SettingsMgt.CheckSetting(ConstMgt.BOS_Print_WhseMovement) then
                                WhsePrint.PrintMovementHeader(WhseActivityHeader)
                            else
                                SetStatusSkipped();
                    end;
                end;
    ...
    begin
        ...
        case PostProcessEntry."Operation Type" of
            PostProcessEntry."Operation Type"::Post:
                begin
            ...
            PostProcessEntry."Operation Type"::Invoice:
                begin
                    WhseShipmentLine.Reset;
                    WhseShipmentLine.SetRange("No.", WhseShipmentHeader."No.");
                    if WhseShipmentLine.FindFirst then begin
            ...
            PostProcessEntry."Operation Type"::Custom:
                begin
                    case PostProcessEntry.Parameters of
                        ConstMgt.PPP_PickFromWhseShipment:
                            begin
    ...
    begin
        ...
        case PostProcessEntry."Operation Type" of
            PostProcessEntry."Operation Type"::Ship:
                begin
            ...
            PostProcessEntry."Operation Type"::Receive:
                begin
            ...
            PostProcessEntry."Operation Type"::Custom:
                begin
                    case PostProcessEntry.Parameters of
                        ConstMgt.PPP_WhseShipFromTransOrder:
                            begin
                        ...
                        ConstMgt.PPP_WhseRcptFromTransOrder:
                            begin
                                Clear(ReleaseTransfer);
                                ReleaseTransfer.Run(TransferHeader);
        ...
        case PostProcessEntry."Operation Type" of
            PostProcessEntry."Operation Type"::Print:
                begin
                    ...
                    if PostedWhseShptLine.FindSet(false, false) then
                        repeat
                            case PostedWhseShptLine."Posted Source Document" of
                                PostedWhseShptLine."Posted Source Document"::"Posted Shipment":
                                    if not SalesShptHeader2Print.Get(PostedWhseShptLine."Posted Source No.") then begin
                                        SalesShptHeader2Print.Init;
                                        SalesShptHeader2Print."No." := PostedWhseShptLine."Posted Source No.";
                                        SalesShptHeader2Print.Insert;
                                    end;
                                PostedWhseShptLine."Posted Source Document"::"Posted Transfer Shipment":
                                    if not TransShptHeader2Print.Get(PostedWhseShptLine."Posted Source No.") then begin
                                        TransShptHeader2Print.Init;
                                        TransShptHeader2Print."No." := PostedWhseShptLine."Posted Source No.";
                                        TransShptHeader2Print.Insert;
                                    end;
                                PostedWhseShptLine."Posted Source Document"::"Posted Return Shipment":
                                    if not ReturnShptHeader2Print.Get(PostedWhseShptLine."Posted Source No.") then begin
                    ...
                    SalesShptHeader2Print.Reset;
                    if SalesShptHeader2Print.FindSet(false, false) then
                        repeat
                            SalesShptHeader.Get(SalesShptHeader2Print."No.");
                            Clear(RecRef);
                            RecRef.GetTable(SalesShptHeader);
                            PostProcessMgt.LogPostProcessOperation(RecRef, enum::DYM_PostProcessOperationType::Print, '', PostProcessEntry."Entry No.", false, '');
                        until SalesShptHeader2Print.Next = 0;
                    TransShptHeader2Print.Reset;
                    if TransShptHeader2Print.FindSet(false, false) then
                        repeat
                            TransShptHeader.Get(TransShptHeader2Print."No.");
                            Clear(RecRef);
                            RecRef.GetTable(TransShptHeader);
                            PostProcessMgt.LogPostProcessOperation(RecRef, enum::DYM_PostProcessOperationType::Print, '', PostProcessEntry."Entry No.", false, '');
                        until TransShptHeader2Print.Next = 0;
                    ReturnShptHeader2Print.Reset;
                    if ReturnShptHeader2Print.FindSet(false, false) then
                        repeat
                            ReturnShptHeader.Get(TransShptHeader2Print."No.");
                            Clear(RecRef);
        ...
        case PostProcessEntry."Operation Type" of
            PostProcessEntry."Operation Type"::Print:
                begin
                    ...
                    if PostedWhseRcptLine.FindSet(false, false) then
                        repeat
                            case PostedWhseRcptLine."Posted Source Document" of
                                PostedWhseRcptLine."Posted Source Document"::"Posted Receipt":
                                    if not PurchRcptHeader2Print.Get(PostedWhseRcptLine."Posted Source No.") then begin
                                        PurchRcptHeader2Print.Init;
                                        PurchRcptHeader2Print."No." := PostedWhseRcptLine."Posted Source No.";
                                        PurchRcptHeader2Print.Insert;
                                    end;
                                PostedWhseRcptLine."Posted Source Document"::"Posted Transfer Receipt":
                                    if not TransRcptHeader2Print.Get(PostedWhseRcptLine."Posted Source No.") then begin
                                        TransRcptHeader2Print.Init;
                                        TransRcptHeader2Print."No." := PostedWhseRcptLine."Posted Source No.";
                                        TransRcptHeader2Print.Insert;
                                    end;
                                PostedWhseRcptLine."Posted Source Document"::"Posted Return Receipt":
                                    if not ReturnRcptHeader2Print.Get(PostedWhseRcptLine."Posted Source No.") then begin
                            ...
                            if PutAwayLine.FindSet(false, false) then
                                repeat
                                    if not PutAwayHeader2Print.Get(PutAwayLine."Activity Type", PutAwayLine."No.") then begin
                    ...
                    if SettingsMgt.CheckSetting(ConstMgt.BOS_Print_PurchReceipt) then begin
                        PurchRcptHeader2Print.Reset;
                        if PurchRcptHeader2Print.FindSet(false, false) then
                            repeat
                                PurchRcptHeader.Get(PurchRcptHeader2Print."No.");
                                Clear(RecRef);
                    ...
                    if SettingsMgt.CheckSetting(ConstMgt.BOS_Print_TransReceipt) then begin
                        TransRcptHeader2Print.Reset;
                        if TransRcptHeader2Print.FindSet(false, false) then
                            repeat
                                TransRcptHeader.Get(TransRcptHeader2Print."No.");
                                Clear(RecRef);
                    ...
                    if SettingsMgt.CheckSetting(ConstMgt.BOS_Print_SalesReturnRcpt) then begin
                        ReturnRcptHeader2Print.Reset;
                        if ReturnRcptHeader2Print.FindSet(false, false) then
                            repeat
                                ReturnRcptHeader.Get(ReturnRcptHeader2Print."No.");
                                Clear(RecRef);
                    ...
                    if SettingsMgt.CheckSetting(ConstMgt.BOS_Print_WhsePutAway) then begin
                        ...
                        if PutAwayHeader2Print.FindSet(false, false) then
                            repeat
                                PutAwayHeader.Get(PutAwayHeader2Print.Type, PutAwayHeader2Print."No.");
                                Clear(RecRef);
    ...
    begin
        ...
        case PostProcessEntry."Operation Type" of
            PostProcessEntry."Operation Type"::Ship, PostProcessEntry."Operation Type"::Invoice, PostProcessEntry."Operation Type"::Post:
                begin
                    ...
                    case SalesHeader."Document Type" of
                        ...
                        SalesHeader."Document Type"::Invoice:
                            begin
                                if (PostProcessEntry."Operation Type" in [PostProcessEntry."Operation Type"::Invoice, PostProcessEntry."Operation Type"::Post]) then
                                    if (SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_SalesInvoice())) then begin
                        ...
                        SalesHeader."Document Type"::"Credit Memo":
                            begin
                                if (PostProcessEntry."Operation Type" in [PostProcessEntry."Operation Type"::Invoice, PostProcessEntry."Operation Type"::Post]) then
                                    if (SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_SalesCreditMemo())) then begin
                                        SalesHeader.Ship := true;
                                        SalesHeader.Invoice := true;
                                    end;
                            end;
                        SalesHeader."Document Type"::"Return Order":
                            begin
                                if (PostProcessEntry."Operation Type" in [PostProcessEntry."Operation Type"::Invoice, PostProcessEntry."Operation Type"::Post]) then
                                    if (SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_SalesReturnOrder())) then begin
                ...
                BEGIN
                    case ItemJnlLine."Entry Type" of
                        ItemJnlLine."Entry Type"::"Positive Adjmt.", ItemJnlLine."Entry Type"::"Negative Adjmt.":
                            IF NOT SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_ItemJournal()) THEN
                                SetStatusSkipped();
                        ItemJnlLine."Entry Type"::Output:
                            if not SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_Prod_Output()) then
                                SetStatusSkipped();
                        ItemJnlLine."Entry Type"::Consumption:
                            if not SettingsMgt.CheckSetting(ConstMgt.BOS_AutoPost_Prod_Consump()) then
                                SetStatusSkipped();
                    end;
    ...
    begin
        ...
        WhseActivityLine.SetRange("Activity Type", WhseActivityHeader.Type);
        WhseActivityLine.SetRange("No.", WhseActivityHeader."No.");
        if WhseActivityLine.FindSet(false, false) then
            repeat
                ReservEntry.Reset;
                ...
                if ReservEntry.FindSet(true, false) then
                    repeat
                        if ((ReservEntry."Quantity (Base)" <> ReservEntry."Qty. to Handle (Base)") or (ReservEntry."Quantity (Base)" <> ReservEntry."Qty. to Invoice (Base)")) then begin
    ...
    #endregion 
    #region Printing
    procedure GetPrintReport(Usage: enum "Report Selection Usage") Result: Integer
    var
        ReportSelection: Record "Report Selections";
    begin
        Clear(Result);
