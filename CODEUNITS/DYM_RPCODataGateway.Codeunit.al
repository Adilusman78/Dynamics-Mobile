codeunit 70142 DYM_RPCODataGateway
{
    //RPC
    trigger OnRun()
    begin
    ...
    procedure ODataWrite(dataIn: Text) Result: Text
    var
    ...
    procedure InsertRPCLogEntry(RequestText: Text) Result: BigInteger
    begin
        CLEAR(Result);
        Clear(RPCLog);
    ...
    begin
        ...
        IF CacheMgt.FindRPCCache(enum::DYM_PacketDirection::Push) THEN
            REPEAT
                XMLMgt.AddElement(DatainNode, CacheMgt.CacheBufferFieldName, CacheMgt.CacheBufferData, DummyNode);
        ...
        IF CacheMgt.FindRPCCache(enum::DYM_PacketDirection::Pull) THEN
            REPEAT
                IF ((LastRecID <> CacheMgt.CacheBufferRecID) OR (DataoutNode.AsXmlElement().IsEmpty)) THEN BEGIN
