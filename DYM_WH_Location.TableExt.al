tableextension 84306 DYM_WH_Location extends Location
{
    fields
    {
    ...
    procedure DYM_CheckZeroSalesQtytoShip(_LocationCode: code[10]): Boolean var
    ...
    procedure DYM_CheckZeroPurchQtytoReceive(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroPurchQtytoReceive);
    end;
    procedure DYM_CheckZeroTransQtytoShip(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroTransQtytoShip);
    end;
    procedure DYM_CheckZeroTransQtytoReceive(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroTransQtytoReceive);
    end;
    procedure DYM_CheckPhysInvJnlQtyPhysInv(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
    ...
    procedure DYM_CheckZeroWhseShipmentQtytoShip(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroWhseShptQtytoShip);
    end;
    procedure DYM_CheckZeroWhseRcptQtytoReceive(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroWhseRcptQtytoRcv);
    end;
    procedure DYM_CheckZeroPickQtytoHandle(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroPickQtytoHandle);
    end;
    procedure DYM_CheckZeroPutAwayQtytoHandle(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroPutAwayQtytoHandle);
    end;
    procedure DYM_CheckZeroMovementQtytoHandle(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroMovementQtytoHandle);
    end;
    procedure DYM_CheckZeroWhseActivityQtytoHandle(_LocationCode: code[10]; _ActivityType: enum "Warehouse Activity Type")Result: Boolean begin
        clear(Result);
        case _ActivityType of _ActivityType::Pick: Result:=DYM_CheckZeroPickQtytoHandle(_LocationCode);
        _ActivityType::"Put-away": Result:=DYM_CheckZeroPutAwayQtytoHandle(_LocationCode);
        _ActivityType::Movement: Result:=DYM_CheckZeroMovementQtytoHandle(_LocationCode);
        end;
    end;
    procedure DYM_CheckZeroWhsePhysInvJnlQtyPhys(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
    ...
    procedure DYM_AutoMobStatusWhseShipment(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_MobStatusWhseShipment);
    end;
    procedure DYM_AutoMobStatusWhseReceipt(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_MobStatusWhseReceipt);
    end;
    procedure DYM_AutoMobStatusWhsePick(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_MobStatusPick);
    end;
    procedure DYM_AutoMobStatusWhsePutAway(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_MobStatusPutaway);
    end;
    procedure DYM_AutoMobStatusWhseMovement(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_MobStatusMovement);
    end;
    procedure DYM_AutoMobStatusWhseActivity(_LocationCode: code[10]; _ActivityType: enum "Warehouse Activity Type")Result: Boolean begin
        clear(Result);
        case _ActivityType of _ActivityType::Pick: Result:=DYM_AutoMobStatusWhsePick(_LocationCode);
        _ActivityType::"Put-away": Result:=DYM_AutoMobStatusWhsePutAway(_LocationCode);
        _ActivityType::Movement: Result:=DYM_AutoMobStatusWhseMovement(_LocationCode);
        end;
    end;
    procedure DYM_AutoMobStatusTransferRcpt(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
    ...
    procedure DYM_CheckZeroSalesTrackQtytoHandle(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroSalesTrackQtytoHandle);
    end;
    procedure DYM_CheckZeroPurchTrackQtytoHandle(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroPurchTrackQtytoHandle);
    end;
    procedure DYM_CheckZeroTransShipQtytoHandle(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
        exit(DMLocation.DYM_ZeroTransShipQtytoHandle);
    end;
    procedure DYM_CheckZeroTransReceiveQtytoHandle(_LocationCode: code[10]): Boolean var
        DMLocation: Record Location;
    begin
        if not DMLocation.GET(_LocationCode)then clear(DMLocation);
