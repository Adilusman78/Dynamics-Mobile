codeunit 84027 DYM_GeneralDataProcess
{
    ...
    begin
        ...
        TableBuffer.Reset;
        if TableBuffer.FindSet(false, false)then repeat if(CRUDType in[ConstMgt.CRD_None, ConstMgt.CRD_Update, ConstMgt.CRD_Delete])then begin
    ...
    begin
        ...
        if FieldMap.FindSet(false, false)then repeat FromField:=TableBuffer.Field(FieldMap."Field No.");
        ...
        case CRUDType of ConstMgt.CRD_None: if not RecRef.Insert(CallTriggers)then RecRef.Modify(CallTriggers);
        ConstMgt.CRD_Create: RecRef.Insert(CallTriggers);
        ConstMgt.CRD_Update: RecRef.Modify(CallTriggers);
        ConstMgt.CRD_Delete: RecRef.Delete(CallTriggers);
        end;
    end;
    #endregion 
    #region Pull
    ...
    begin
        ...
        RecRef.Open(TableMap."Table No.");
        RecRef.Reset;
        if(TableMap.Key = 0)then PullProcessAutoSelectKey(RecRef, TableMap)
        else
            RecRef.CurrentKeyIndex(TableMap.Key);
        Clear(HasFieldFilters);
        HasFieldFilters:=PullProcessApplyStaticFilters(RecRef, TableMap, false, DummyFieldMap);
        case HasFieldFilters of false: begin
            if RecRef.FindSet(false, false)then repeat Counter+=1;
        ...
        true: begin
            ...
            if ParentTableData.FindSet(false, false)then repeat TableMapFilters.Reset;
                    ...
                    if TableMapFilters.FindSet(false, false)then repeat Evaluate(FieldValue, TableMapFilters."Filter Value");
    ...
    begin
        ...
        if FieldMap.FindSet(false, false)then repeat PullProcessApplyStaticFilters(RecRef, TableMap, true, FieldMap);
    ...
    procedure PullProcessApplyStaticFilters(var SourceTable: RecordRef; var TableMap: Record DYM_MobileTableMap; FlowFiltersOnly: Boolean; var FieldMap: Record DYM_MobileFieldMap)Result: Boolean var
        TableMapFilters: Record DYM_MobileTableMapFilters;
        FromField: FieldRef;
    begin
        Clear(Result);
        ...
        if TableMapFilters.FindSet(false, false)then repeat FromField:=SourceTable.Field(TableMapFilters."Filter Field No.");
                case TableMapFilters."Filter Type" of TableMapFilters."Filter Type"::Const: LowLevelDP.Value2FldRefFilter(FromField, TableMapFilters."Filter Value");
                TableMapFilters."Filter Type"::Filter: FromField.SetFilter(SessionMgt.ApplySessionValuesToFilter(TableMapFilters."Filter Value"));
                TableMapFilters."Filter Type"::"Session Value": FromField.SetFilter(SessionMgt.GetSessionValue(TableMapFilters."Filter Value"));
                TableMapFilters."Filter Type"::Field: begin
                    if((TableMap."Parent Table No." = 0) or (TableMap."Parent Table Index" = 0))then TableMapFilters.FieldError("Filter Type");
                    Result:=true;
                end;
                TableMapFilters."Filter Type"::"Setting Value": begin
                    FromField.SetFilter(SettingsMgt.GetSetting(TableMapFilters."Filter Value"));
                end;
                end;
            until TableMapFilters.Next = 0;
    end;
    procedure PullProcessExistRelation(var ParentTableData: RecordRef; var TableMap: Record DYM_MobileTableMap)Result: Boolean var
    ...
    begin
        ...
        if ChildTableMap.FindSet(false, false)then begin
            repeat Clear(Result);
                ...
                case ChildTableMap."Relation Type" of ChildTableMap."Relation Type"::Exists: exit(not RecRef.IsEmpty);
                ChildTableMap."Relation Type"::"Not Exists": exit(RecRef.IsEmpty);
                end;
    ...
    procedure PullProcessLookUpRelation(var ParentTableData: RecordRef; var TableMap: Record DYM_MobileTableMap; var FieldMap: Record DYM_MobileFieldMap)Result: Text[250]var
        ChildTableMap: Record DYM_MobileTableMap;
        TableMapFilters: Record DYM_MobileTableMapFilters;
        DummyFieldMap: Record DYM_MobileFieldMap;
    ...
    procedure PullProcessAutoSelectKey(var SourceTable: RecordRef; var TableMap: Record DYM_MobileTableMap)Result: Boolean var
        TableMapFilters: Record DYM_MobileTableMapFilters;
    ...
    begin
        ...
        if TableMapFilters.FindSet(false, false)then repeat FromField:=SourceTable.Field(TableMapFilters."Filter Field No.");
                KeyBuffer.Reset;
                if KeyBuffer.FindSet(false, false)then repeat KeyRef:=SourceTable.KeyIndex(KeyBuffer."Entry No.");
    ...
    #endregion 
    #region LookUp
    procedure LookUpTable(var TableNo: Integer)Result: Boolean var
    ...
    procedure LookUpField(TableNo: Integer; var FieldNo: Integer)Result: Boolean var
        Fld: Record "Field";
