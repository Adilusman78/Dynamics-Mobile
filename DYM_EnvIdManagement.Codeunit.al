codeunit 84040 DYM_EnvIdManagement
{
    ...
    begin
        ...
        case((LocalEnvTSH = CloudEnvTSH) OR (LocalEnvTSH = ''))of true: //Environment TimeStamp Hash match. Everything is fine.
 begin
            clear(NewEnvTSH);
            NewEnvTSH:=genEnvTSH();
 ...
 begin
            case(NextLocalEnvTSH = CloudEnvTSH)of true: //Pseudo poke fail detected. 
 begin
 ...
 begin
    ...
    local procedure genEnvId(): Text begin
    ...
    local procedure genNextEnvId(): Text begin
        SettingsMgt.TestGlobalSetting(ConstMgt.CLD_AppArea());
        exit(StrSubstNo(ConstMgt.CLD_NExtEnvIdPattern(), SettingsMgt.GetGlobalSetting(ConstMgt.CLD_AppArea())));
    end;
    local procedure genEnvTSH(): Text begin
        exit(CreateGuid());
    end;
    local procedure peekEnvTSH(EnvId: Text)Result: Text var
        client: HttpClient;
    ...
    local procedure pokeEnvTSH(EnvId: Text; EnvTSH: Text): Boolean var
        client: HttpClient;
        request: HttpRequestMessage;
        response: HttpResponseMessage;
        headers: HttpHeaders;
    ...
    begin
        ...
        IF JobQueueEntry.FINDSET(FALSE, FALSE)THEN REPEAT JobQueueEntry2.GET(JobQueueEntry.ID);
