codeunit 84030 DYM_IntegrityManagement
{
    var Text001: Label '%1 modules updated to Integrity Modules list.';
    ...
    begin
        ...
        InstalledApps.Reset;
        If InstalledApps.FindSet(false, false)then repeat NavApp.GetModuleInfo(InstalledApps."App ID", AppModuleInfo);
    ...
    procedure TestModulesIntegrity(_Silent: Boolean)Result: Boolean var
    ...
    begin
        Result:=true;
        IntegrityInfo.Reset;
        if IntegrityInfo.FindSet(false, false)then repeat if not InstalledApps.Get(IntegrityInfo."App Id")then clear(Result);
