codeunit 84043 DYM_CryptoManagement
{
    ...
    var Polynomial: Label 'A001', Locked = true;
    CRCTable: array[256]of Integer;
    bit: Codeunit DYM_Bitwise;
    procedure computeCRC(textValue: Text): Integer var
        i, index, crc: Integer;
        textValueChar: Byte;
    begin
        generateCRCTable();
    ...
    begin
        for tableLoop:=0 to(ArrayLen(CRCTable) - 1)do begin
            ...
            for bitLoop:=0 to 7 do begin
                if(bit.bitAnd16(bit.bitXor16(cellValue, tempValue), bit.bitHex16('0001')) <> 0)then cellValue:=bit.bitXor16(bit.bitShr16(cellValue, 1), bit.bitHex16(Polynomial))
                else
    ...
    procedure getCRCTableData(index: integer): Integer begin
        exit(CRCTable[index]);
    end;
}
