codeunit 84033 DYM_CloudManagement
{
    TableNo = DYM_SyncLog;
 ...
 processFetch()
            ...
            begin
                case Rec.Direction of Rec.Direction::Pull: processPull(Rec);
                Rec.Direction::Push: processPush(Rec);
                end;
            end;
        end
        else
        begin //Process BLOBStore entry
    ...
    local procedure getDownloadLink(_packetId: Text): Text var
        client: HttpClient;
    ...
    local procedure parseDownloadLink(_downloadLink: Text): Text var
        json: JsonObject;
    ...
    local procedure getUploadLink(_packetId: Text; _deviceId: Text; _packetSize: Integer): Text var
        client: HttpClient;
        request: HttpRequestMessage;
        response: HttpResponseMessage;
        headers: HttpHeaders;
    ...
    local procedure parseUploadLink(_uploadLink: Text): Text var
        json: JsonObject;
    ...
    procedure fetchPacketsList(_packetType: Text): Text var
        client: HttpClient;
        request: HttpRequestMessage;
        response: HttpResponseMessage;
        headers: HttpHeaders;
    ...
    begin
        ...
        if(response.Content.ReadAs(respBodyText))then begin
            ...
            Message(StrSubstNo('[%1][%2]', response.HttpStatusCode(), respBodyText))end
        else
            Message('Cannot read body');
        //EventLogMgt.LogProcessDebug_MethodEnd();
        EventLogMgt.LogProcessDebug_APICallDuration(PerfCounterId);
    ...
    procedure MarkPacketsList(_PacketsList: Text; _packetType: Text): Text var
        client: HttpClient;
        request: HttpRequestMessage;
        response: HttpResponseMessage;
        headers: HttpHeaders;
    ...
    #endregion 
    procedure testConnection(TestAppArea: Text; TestxAPIKey: Text; var ResultMsg: Text)Result: Boolean var
        client: HttpClient;
        request: HttpRequestMessage;
        response: HttpResponseMessage;
        headers: HttpHeaders;
    ...
    procedure ApplyPlaceHolderValue(InputString: Text; PlaceHolder: Text; PlaceHolderValue: Text)Result: Text begin
